...
type ReturnType = {
  get: <T extends unknown>(url: string, query?: Record<string, any>) => Promise<TApiResponse<T>>;
  post: <T extends unknown>(url: string, payload: Record<string, number | string | any>) => Promise<TApiResponse<T>>;
};
...
const useApi = (): ReturnType => {
  const request = async <T = any,>(method: TMethod, path: string, payload?: any, query?: Record<string, any>): Promise<TApiResponse<T>> => {
    try {
